...
pub = rospy.Publisher('from_speed_diagram', std_msgs.msg.Int16MultiArray, queue_size=10)
mat_pub = std_msgs.msg.Int16MultiArray()
...
def speed_polar_diagram_calculation_node(mat_sub):
    global mat_pub
    w_speed = mat_sub.data[0]
    w_theta = mat_sub.data[1]
    ...
    print(max_vel)
    mat_pub.data = [max_vel, max(up_beat[w_speed_index] + w_theta), max(dn_beat[w_speed_index] + w_theta)]
    print(mat_pub.data)
...
"""
def speed_polar_diagram_server():
    rospy.init_node('speed_polar_diagram', anonymous=True)
    rospy.Subscriber("to_speed_diagram", std_msgs.msg.Int16MultiArray, speed_polar_diagram_calculation)
