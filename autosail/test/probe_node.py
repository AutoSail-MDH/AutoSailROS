    ...
    while not rospy.is_shutdown():
        ...
        for pub_test in pub_tests:
            ...
            if not pub_test.msg_received:
                rospy.logerr(f"{pub_test.topic} did not publish any msgs for {time_now - pub_test.last_msg_time}s")
            else:
                msg = pub_test.msg
                if hasattr(msg, 'header'):
                    delattr(msg, 'header')
                if pub_test is not None and msg == pub_test.last_msg:
                    rospy.logwarn(f"""{pub_test.topic} did not change since last check, 
                                  {time_now - pub_test.last_msg_time}s ago""")
